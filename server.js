...
mongoose.connect(mongoURI)
    .then(() => {
        ...
        // Les index seront créés automatiquement via le schéma
    })
...
// ============================================
// MODÈLES DE DONNÉES AVEC INDEX (Amendement VII)
// ============================================
...
// Index pour le genotype (Amendement VII)
...
// Index composé pour les requêtes de messages (Amendement VII)
...
app.get('/login', (req, res) => {
    res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Connexion - Genlove</title>' + styles + '</head><body><div class="app-shell"><div class="page-white"><h2>Connexion</h2><p style="color:#666; margin:20px;">Pour la démo, utilisez le bouton ci-dessous</p><a href="/profile" class="btn-pink">Accéder à mon profil</a><a href="/" class="btn-dark" style="margin-top:15px;">Retour</a></div></div></body></html>');
});
...
app.get('/matching', requireAuth, requireVerified, async (req, res) => {
    try {
        ...
        res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Matching - Genlove</title>' + styles + '</head><body><div class="app-shell"><div class="page-white"><h2>Partenaires compatibles</h2>' + (partnersHTML || '<p style="color:#666; padding:30px;">Aucun partenaire trouvé pour le moment</p>') + '<a href="/profile" class="btn-pink">Retour au profil</a></div></div></body></html>');
    } catch (error) {
...
app.get('/blocked-list', requireAuth, requireVerified, async (req, res) => {
    try {
        ...
        res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Utilisateurs bloqués</title>' + styles + '</head><body><div class="app-shell"><div class="page-white"><h2>Utilisateurs bloqués</h2>' + (blockedHTML || '<p>Aucun utilisateur bloqué</p>') + '<a href="/settings" class="btn-pink">Retour</a></div></div><script>async function unblockUser(id){await fetch("/api/unblock/"+id,{method:"POST"});location.reload();}</script></body></html>');
    } catch (error) {
