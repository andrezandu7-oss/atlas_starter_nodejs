...
app.get('/matching', (req, res) => {
    ...
    <div id="popup-overlay" onclick="closePopup()">
        <div class="popup-content" onclick="event.stopPropagation()">
            ...
            <button class="btn-pink" style="margin:20px 0 0 0; width:100%" onclick="startChat(); closePopup(); showNotify('Conversation démarrée !')">🚀 Contacter ce profil</button>
        </div>
    </div>
    ${notifyScript}
    <script>
        const partners = [
            {id:1, gt:"AA", gs:"O+", pj:"Désire fonder une famille unie.", name:"Sarah"},
            {id:2, gt:"AS", gs:"B-", pj:"Souhaite des enfants en bonne santé.", name:"Aminata"},
            {id:3, gt:"SS", gs:"A+", pj:"Cherche une relation stable et sérieuse.", name:"Fatou"}
        ];
        ...
        filtered.forEach(p => {
            container.innerHTML += `
                <div class="match-card">
                    ...
                    <div style="flex:1"><b>${p.name} (#${p.id})</b><br><small>Génotype ${p.gt}</small></div>
        ...
        function startChat() {
            sessionStorage.setItem('chatPartner', JSON.stringify(selectedPartner));
...
// === MESSAGERIE/NOTIFICATIONS (ex-2ème app) ===
app.get('/chat', (req, res) => {
    const partner = JSON.parse(sessionStorage.getItem('chatPartner') || '{"id":1, "name":"Sarah", "gt":"AA", "gs":"O+", "pj":"Désire fonder une famille unie."}');
    res.send(`
<!DOCTYPE html>
<html lang="fr">
<head>
