// ============================================
// GENLOVE V4.5 - MESSAGERIE PERMANENTE + PWA
// ============================================
...
// Vérifier compatibilité génétique
function estCompatible(genotype1, genotype2) {
  if (!genotype1 || !genotype2) return false;
  
  // Si utilisateur est AS ou SS → ne peut voir que AA
  if ((genotype1 === 'AS' || genotype1 === 'SS') && genotype2 !== 'AA') {
    return false;
  }
  
  // Si utilisateur est AA → peut voir tout le monde
  if (genotype1 === 'AA') return true;
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    await newUser.save();
    console.log("✅ ENREGISTRÉ:", firstName);
...
app.delete('/api/delete-account/:id', async (req, res) => {
  try {
    ...
    // Supprimer aussi tous les messages de l'utilisateur
...
app.post('/api/messages', async (req, res) => {
  try {
    ...
    const message = new Message({
      senderId, receiverId, text, timestamp: new Date()
    });
    
    await message.save();
    res.json({ success: true, message: "Message envoyé", data: message });
  } catch (error) {
...
app.get('/api/conversations/:userId', async (req, res) => {
  try {
    ...
    // Extraire les IDs uniques des interlocuteurs
...
app.get('/manifest.json', (req, res) => {
  res.json({
    name: "Genlove",
    short_name: "Genlove",
    description: "Rencontres basées sur la compatibilité génétique",
    start_url: "/",
    display: "standalone",
    ...
    icons: [
      {
        src: "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90' fill='%23ff416c'>💙</text></svg>",
        sizes: "192x192",
        type: "image/svg+xml"
      },
      {
...
app.get('/sw.js', (req, res) => {
  ...
  res.send(`
    ...
    const urlsToCache = ['/', '/profile', '/matching', '/inbox'];
            ...
            <form onsubmit="saveAndRedirect(event)">
              ...
              <div style="display:flex;gap:10px;">
                <select id="gs_type" class="input-box" style="flex:2;">
                  <option value="">Groupe</option><option>A</option><option>B</option><option>AB</option><option>O</option>
                </select>
...
// ✅ Matching avec filtrage génétique strict
      ...
      <html>
        ...
        <body style="background:#f4f7f6;">
          ...
          <script>
            ...
            // Filtrer selon compatibilité génétique et blocages
            window.onload = function() {
              try {
                ...
                cards.forEach(card => {
                  ...
                  // Ne pas se voir soi-même
                  if (pUserId === myId) visible = false;
                  
                  // Ne pas voir le même genre
                  if (myGender && pGender === myGender) visible = false;
                  
                  // RÈGLE DE COMPATIBILITÉ STRICTE
                  if (myGt === 'AS' || myGt === 'SS') {
                    if (pGt !== 'AA') visible = false; // AS/SS ne voit que AA
                  }
                  // Si AA peut voir tout le monde (déjà filtré par genre)
                ...
                // Message si aucun résultat pour AS/SS
      ...
      <body>
        ...
        <script>
          ...
          window.onload = async function() {
            ...
            loadMessages();
            refreshInterval = setInterval(loadMessages, 3000); // Rafraîchir toutes les 3 secondes
          };
...
// ✅ Route pour obtenir un utilisateur par ID (pour le chat)
app.get('/api/users/:id', async (req, res) => {
  try {
    const user = await User.findById(req.params.id).select('firstName lastName photo genotype');
    if (!user) {
      return res.status(404).json({ error: "Utilisateur non trouvé" });
    }
...
// ✅ Route pour les utilisateurs bloqués
...
// ✅ Route de fin (gardée pour compatibilité)
...
// ✅ Route logout (gardée pour compatibilité)
...
app.listen(port, '0.0.0.0', () => {
  ...
  console.log("✅ NOUVEAUTÉS V4.5:");
  console.log("  • Messagerie permanente avec MongoDB");
  console.log("  • Boîte de réception (/inbox)");
  console.log("  • Règles compatibilité strictes (AS/SS → AA seulement)");
  console.log("  • Blocage utilisateurs");
  console.log("  • Suppression conversations");
  console.log("  • Installation PWA (manifest + service worker)");
  console.log("  • Nettoyage complet (plus de web-push, plus d'éphémère)");
});