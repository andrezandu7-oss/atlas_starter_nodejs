...
// --- PARTIE 1 : STYLES ET SCRIPTS DU CORE APP ---
const styles = `
<style>
...
app.get('/profile', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0">${styles}</head><body style="background:#f8f9fa;"><div class="app-shell"><div style="background:white; padding:30px 20px; text-align:center; border-radius:0 0 30px 30px;"><div style="display:flex; justify-content:space-between; align-items:center;"><a href="/" style="text-decoration:none; background:#eff6ff; color:#1a2a44; padding:8px 14px; border-radius:12px; font-size:0.8rem; font-weight:bold; display:flex; align-items:center; gap:8px; border: 1px solid #dbeafe;"><span style="font-size:1rem;">🏠</span> Accueil</a><a href="/settings" style="text-decoration:none; font-size:1.4rem;">⚙️</a></div><div id="vP" style="width:110px; height:110px; border-radius:50%; border:3px solid #ff416c; margin:20px auto; background-size:cover;"></div><h2 id="vN">Utilisateur</h2><p id="vR" style="color:#666; font-size:0.9rem;">📍 Localisation</p><p style="color:#007bff; font-weight:bold;">Profil Santé Validé ✅</p></div><div class="st-group"><div class="st-item"><span>Génotype</span><b id="rG">...</b></div><div class="st-item"><span>Groupe Sanguin</span><b id="rS">...</b></div><div class="st-item"><span>Projet de vie</span><b id="rP">...</b></div></div><a href="/matching" class="btn-dark" style="text-decoration:none;">🔍 Trouver un partenaire</a></div>
    <script>const p = localStorage.getItem('u_p'); if(p) document.getElementById('vP').style.backgroundImage = 'url('+p+')'; document.getElementById('vN').innerText = (localStorage.getItem('u_fn') || "") + " " + (localStorage.getItem('u_ln') || ""); document.getElementById('vR').innerText = "📍 " + (localStorage.getItem('u_res') || ""); document.getElementById('rG').innerText = localStorage.getItem('u_gt'); document.getElementById('rS').innerText = localStorage.getItem('u_gs'); document.getElementById('rP').innerText = "Enfant : " + localStorage.getItem('u_pj');</script></body></html>`);
});

app.get('/matching', (req, res) => {
    res.send(`<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0">${styles}</head>
    <body style="background:#f4f7f6;"><div class="app-shell">
        <div id="genlove-notify" style="position:fixed;"><span>💙</span><span id="notify-msg"></span></div>
    ...
    <div id="popup-overlay" onclick="closePopup()">
        <div class="popup-content" onclick="event.stopPropagation()">
            ...
            <div id="pop-details" style="font-size:0.95rem; line-height:1.6;"></div>
            <div id="pop-msg" class="popup-msg"></div>
            <button class="btn-pink" style="margin-top:20px; width:100%" onclick="window.location.href='/simulation'">🚀 Contacter ce profil</button>
        </div>
    </div>
    <script>
        const partners = [{id:1, gt:"AA", gs:"O+", pj:"Désire fonder une famille unie."},{id:2, gt:"AS", gs:"B-", pj:"Souhaite des enfants sains."},{id:3, gt:"SS", gs:"A+", pj:"Cherche relation stable."}];
        ...
        filtered.forEach(p => {
            container.innerHTML += \`<div class="match-card"><div class="match-photo-blur"></div><div style="flex:1"><b>Profil #\${p.id}</b><br><small>Génotype \${p.gt}</small></div><button class="btn-action btn-details" onclick='showDetails(\${JSON.stringify(p)})'>Détails</button></div>\`;
        });
        function showDetails(p) {
            document.getElementById('pop-name').innerText = "Profil #" + p.id;
            document.getElementById('pop-details').innerHTML = "<b>Génotype :</b> " + p.gt + "<br><b>Groupe :</b> " + p.gs + "<br><i>" + p.pj + "</i>";
            let msg = (myGt === "AA" && p.gt === "AA") ? "<b>L'Union Sérénité</b>" : (myGt === "AA" && p.gt === "SS") ? "<b>L'Union Solidaire</b>" : "<b>L'Union Protectrice</b>";
...
// --- PARTIE 3 : LA SIMULATION (GENLOVE APP) ---
...
<head>
    ...
    <style>
        ...
        .btn-quit { background: white; color: #9dbce3; border: none; width: 32px; height: 32px; border-radius: 8px; font-weight: bold; cursor: pointer; }
        ...
        .digital-clock { background: #1a1a1a; color: #ff416c; padding: 6px 15px; border-radius: 10px; font-family: monospace; font-size: 1.1rem; display: inline-flex; align-items: center; border: 1px solid #333; }
        ...
        .input-area { position: fixed; bottom: 0; width: 100%; max-width: 450px; padding: 10px 15px 45px; border-top: 1px solid #eee; display: flex; gap: 10px; background: white; box-sizing: border-box; }
        ...
        .btn-restart { background: #ff416c; color: white; border: none; padding: 16px; border-radius: 30px; width: 100%; font-weight: bold; margin-top: 25px; cursor: pointer; }
...
<body>
    ...
    <div id="screen1" class="screen active" style="background:#f0f2f5; justify-content:center; align-items:center;">
        <div style="background:white; width:85%; border-radius:20px; padding-bottom:20px; overflow:hidden; text-align:center; box-shadow:0 4px 15px rgba(0,0,0,0.1);">
            <div style="padding:15px; border-bottom:1px solid #eee; font-weight:bold;">📩 Genlove Notification</div>
            <p style="padding:20px;">Quelqu'un de compatible souhaite échanger 💞</p>
            <button style="background:#7ca9e6; color:white; border:none; width:90%; padding:15px; border-radius:12px; font-weight:bold; cursor:pointer;" onclick="show(2)">📖 Ouvrir Genlove</button>
        </div>
    </div>
    ...
    <div id="screen3" class="screen">
        <div id="security-popup"><div class="popup-card"><h3>🔒 Discussion privée</h3><p>Tout s'efface dans 30 min.</p><div class="pedagogic-box">🛡️ Éphémère | 🕵️ Privé</div><button style="background:#4a76b8; color:white; border:none; padding:16px; border-radius:30px; width:100%; font-weight:bold;" onclick="closePopup()">Démarrer</button></div></div>
